...
This simple extension exposes a command (which can be invoked from the Command Pallete or by assigning a Keyboard Shortcut) that looks for a markdown checkbox on the current line, the cycles between four states: unchecked `[ ]`, in-progress `[~]`, finished `[x]`, and marked `[*]`.
